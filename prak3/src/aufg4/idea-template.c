...
int main(void)
{
  ...
  for(i=0; i<length; i+=8)
  {
    idea_cbc_encrypt(&input[i],&output[i],8,&decrypt_ks, tmp, IDEA_DECRYPT);
  }
  printf("%s\n", output);
  ...
  i=0;
  memcpy(tmp, iv, 8);
  idea_cfb64_encrypt(input, output, length, &encrypt_ks, tmp, &i, IDEA_DECRYPT);
  ...
  memcpy(tmp, iv, 8);
  idea_ofb64_encrypt(input, output, length, &encrypt_ks, tmp, &i);
  printf("%s\n", output);

  /* read 512 bytes from file t5 */
  /*read(t5, input, length);
  for(i=0; i<length; i+=8)
  {
    idea_ecb_encrypt(&input[i],&output[i],&decrypt_ks);
  }*/
